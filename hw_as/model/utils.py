...
__all__ = ["SincConv_fast", "ResBlock"]
...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=16000, in_channels=1,
                 stride=1, padding=0, dilation=1, bias=False, groups=1, min_low_hz=50, min_band_hz=50, abs_out=True):
    ...
    def forward(self, waveforms):
        ...
        result = F.conv1d(waveforms, self.filters, stride=self.stride,
                        padding=self.padding, dilation=self.dilation,
                        bias=None, groups=1)
        return torch.abs(result) if self.abs_out else result
